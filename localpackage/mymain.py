...
c1 = {"name": "Griesbach", "cont": 1, "age": 61, "sex": "Male", 'dod': '13/4/2021', 'fatal': False,
...
mason = {"name": "Mason", "cont": 1, "dob": '21/10/2001', 'deltaLE': -3.5, "sex": "Male", 'fatal': False, "dataSet": Ogden8,
...
deceased = {'name': "deceased", 'dob': '25/7/1990', 'dod':'3/07/2023' , 'fatal': True, 'sex': 'Male',
            'dataSet': Ogden8, 'targetLE': 20, 'retirement': 78, 'cont': 0.75}
...
eg = {"rows": [], 'game': {"trialDate": datetime(2026, 7, 25), "DOI": datetime(2011, 6, 13), "useMultipleRates": False,
                           'DRMethod': 'SINGLE', "projection": True, 'useTablesEF': True, "autoYrAttained": False,
                           "discountRate": -0.25 / 100, "Ogden": 8,
                           "multipleRates": [{'rate': -0.015, 'switch': 15}, {'rate': 0.015, 'switch': 125}],
                           "claimants": [dependent, deceased]}}

#eg={'game':{"discountRate":-0.0025,"useTablesEF":False,"autoYrAttained":False,"claimants":[c1],"useMultipleRates":False,"DRMethod":"SINGLE","DOS":"1/08/2023","trialDate":"22/07/2024","projection":True,"Ogden":8,"multipleRates":[{"rate":-0.015,"switch":15},{"rate":0.015,"switch":125}],"DOI":"16/04/2021"}}
...
def joint_lives_tablev2(start_age, end_age, sexes):
    ...
    ages = [*range(0, 101)]
    men = [{'name': "man " + str(age) + " " + str(dage), 'age': age, 'sex': sexes[1], 'dataSet': Ogden8,
            'dependenton': 'woman ' + str(dage)} for age in ages for dage in ages]  # create women of age age
    women = [{'name': "woman " + str(age), 'age': age, 'sex': sexes[0], 'dataSet': Ogden8} for age in
             ages]  # create men of age age

    claimants = men + women

    eg = {"rows": [], 'game': {"trialDate": datetime(2022, 8, 25), "projection": True, "autoYrAttained": False,
                               "discountRate": -0.25 / 100, "Ogden": 8, "claimants": claimants}}
    g = game(eg)
...
def createTableE():
    ...
    men = [{'name': "male " + str(age), 'age': age, 'sex': "M", 'dataSet': Ogden8} for age in
           agedeath]  # create women of age age
    women = [{'name': "female " + str(age), 'age': age, 'sex': "F", 'dataSet': Ogden8} for age in
             agedeath]  # create men of age age
    ...
    dataMale = [[gMen.getClaimant(man).M('TRIAL', 'TRIAL+' + str(c) + "Y", options='M')[3] / c for c in yrstotrial] for
                man in gMen.getClaimants()]
    dataFemale = [
        [gWomen.getClaimant(woman).M('TRIAL', 'TRIAL+' + str(c) + "Y", options='M')[3] / c for c in yrstotrial] for
        woman in gWomen.getClaimants()]
    ...
    write_to_gsheet(service_file_path, spreadsheet_id_generated, 'Table E (male)', dfMale, "Table E (male)",
                    "Yrs to trial", "Age at death")
    write_to_gsheet(service_file_path, spreadsheet_id_generated, 'Table E (female)', dfFemale, "Table E (female)",
                    "Yrs to trial", "Age at death")
...
def createTableF():
    ...
    dataMale = [[gMen.getClaimant(man).M('TRIAL+' + str(c) + "Y", options='M')[3] for c in yrstotrial] for man in
                gMen.getClaimants()]
    dataFemale = [[gWomen.getClaimant(woman).M('TRIAL+' + str(c) + "Y", options='M')[3] for c in yrstotrial] for woman
                  in gWomen.getClaimants()]
    dfMale = pd.DataFrame(dataMale, index=agedeath).set_axis(yrstotrial, 'columns')
    dfFemale = pd.DataFrame(dataFemale, index=agedeath).set_axis(yrstotrial, 'columns')
    print('Writing tables...')
    write_to_gsheet(service_file_path, spreadsheet_id_generated, 'Table F (male)', dfMale, "Table F (male)",
                    "Yrs to trial", "Age at death")
    write_to_gsheet(service_file_path, spreadsheet_id_generated, 'Table F (female)', dfFemale, "Table F (female)",
                    "Yrs to trial", "Age at death")
...
def createTables1to34():
    ...
    tables = [['TRIAL', 'LIFE', [0, 100]], ['TRIAL', 50, [16, 49]], ['TRIAL', 55, [16, 54]], ['TRIAL', 60, [16, 59]],
              ['TRIAL', 65, [16, 64]], ['TRIAL', 68, [16, 67]], ['TRIAL', 70, [16, 69]], ['TRIAL', 75, [16, 74]],
              ['TRIAL', 80, [16, 79]], [50, 'LIFE', [0, 50]], [55, 'LIFE', [0, 55]], [60, 'LIFE', [0, 60]],
              [65, 'LIFE', [0, 65]], [68, 'LIFE', [0, 68]], [70, 'LIFE', [0, 70]], [75, 'LIFE', [0, 75]],
              [80, 'LIFE', [0, 80]]]
    ages = [*range(0, 101)]
    men = [{'name': "male " + str(age), 'age': age, 'sex': "M", 'dataSet': Ogden8} for age in
           ages]  # create women of age age
    women = [{'name': "female " + str(age), 'age': age, 'sex': "M", 'dataSet': Ogden8} for age in
             ages]  # create men of age age
    egMen = {"rows": [], 'game': {"trialDate": datetime(2022, 8, 25), "projection": True, "autoYrAttained": False,
                                  "discountRate": -0.25 / 100, "Ogden": 8, "claimants": men}}
    egWomen = {"rows": [], 'game': {"trialDate": datetime(2022, 8, 25), "projection": True, "autoYrAttained": False,
    ...
    gMen = game(egMen)
    gWomen = game(egWomen)
    dataMen = [[[gMen.getClaimant(c).M(table[0], table[1], discountRate=dr)[3] for dr in drs] for c in gMen.claimants if
                gMen.getClaimant(c).getAge() >= table[2][0] and gMen.getClaimant(c).getAge() <= table[2][1]] for table
               in tables]
    dataWomen = [
        [[gWomen.getClaimant(c).M(table[0], table[1], discountRate=dr)[3] for dr in drs] for c in gWomen.claimants if
         gWomen.getClaimant(c).getAge() >= table[2][0] and gWomen.getClaimant(c).getAge() <= table[2][1]] for table in
        tables]
    dataFramesMen = [
        pd.DataFrame(d, index=[*range(tables[dataMen.index(d)][2][0], tables[dataMen.index(d)][2][1] + 1)]).set_axis(
            drs, 'columns') for d in dataMen]
    dataFramesWomen = [pd.DataFrame(d, index=[
        *range(tables[dataWomen.index(d)][2][0], tables[dataWomen.index(d)][2][1] + 1)]).set_axis(drs, 'columns') for d
                       in dataMen]
...
def createDFTables35to36():
    ...
    g = game(eg)
    dataDF = [[g.getClaimant('male').M("TRIAL + " + str(yr) + "Y", options="A", discountRate=dr)[3] for dr in drs] for
              yr in yrs]
    dataTC = [
        [g.getClaimant('male').M("TRIAL", "TRIAL + " + str(yr) + "Y", options="A", discountRate=dr)[3] for dr in drs]
        for yr in yrs]
    dfDF = pd.DataFrame(dataDF, index=yrs).set_axis(drs, 'columns')
    dfTC = pd.DataFrame(dataTC, index=yrs).set_axis(drs, 'columns')
    print('Writing tables...')
    write_to_gsheet(service_file_path, spreadsheet_id_generated, 'Table 35', dfDF, 'Table 35: discount factor',
                    "Discount rate", "Years")
    write_to_gsheet(service_file_path, spreadsheet_id_generated, 'Table 36', dfTC, 'Table 36: term certain',
                    "Discount rate", "Years")
...
def copypasteOgden8publishedTables(spreadsheet_id):
    # test
    rows = [101, 101, 34, 34, 39, 39, 44, 44, 49, 49, 52, 52, 54, 54, 59, 59, 64, 64, 51, 51, 56, 56, 61, 61, 66, 66,
            69, 69, 71, 71, 76, 76, 81, 81, 80, 80]
    cols = 14
    tables = [*range(1, 37)]
    gc = pygsheets.authorize(service_file=service_file_path)
    ...
    for table in tables:
        ...
        pygsheets.DataRange((rows[table - 1] + 4, 2), (2 * rows[table - 1] + 4, cols + 1),
                            worksheet=wks_generated).apply_format(model_cell)
...
def check_in_list(unique_code):
    ...
    codes = [wks.get_col(2, include_tailing_empty=False), wks.get_col(4, include_tailing_empty=False),
             wks.get_col(6, include_tailing_empty=False)]  # the three codes
    col = 1
    for code in codes:
        col += 2
...
def do(c):
    # g.getClaimant(c).showLEPlot()
    if c=='Mason':
        lump_sum=12909031
#        lump_sum=12543800
    else:
        lump_sum=8666804
    g.getClaimant(c).showCapitalLeftPlot(lump_sum,190000,-0.75/100, 1/100)
    print(g.getClaimant(c).getDOB())
    print(g.getClaimant(c).LE())
    print(g.getClaimant(c).LM())
    print(g.getClaimant(c).getRevisedAge())
    print(g.getClaimant(c).getAge())
...
print (g.getClaimant("deceased").getRevisedAge())
...
#print(g.getClaimant('Mason').LM())
#print(g.getClaimant('Mason').getRevisedAge())

#print(g.getClaimant('Mason').getAge())



#print(g.getClaimant('Jacqueline').M('TRIAL', 'LIFE', 'Y', 'M', overrides="{Age:20}"))
#print(g.getClaimant('Jacqueline').M('TRIAL', 'TRIAL+20Y', options='A', DRMethodOverride='SWITCHED'))
#print(g.getClaimant('Jacqueline').M('TRIAL', 'LIFE'))
#print(g.getClaimant('Jacqueline').M('TRIAL', 'LIFE',                                    overrides="{SEX:FEMALE, AGE:50, DRMETHOD: BLENDED, LONGRATE: 0.05, SHORTRATE: 0.0025}"))
#print(g.getClaimant('Jacqueline').M('TRIAL', 'LIFE', overrides="{SEX:FEMALE, AGE:50, DRMETHOD: BLENDED, LONGRATE: 0.05}"))
#print(g.getClaimant('Jacqueline').LM(overrides="{SEX:FEMALE, AGE:50}"))
#print(g.getClaimant('Jacqueline').LM(overrides="{SEX:FEMALE}"))
