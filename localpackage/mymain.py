...
eg={"rows": [], 'game': {"trialDate":datetime(2022,8,25),"projection":True,"autoYrAttained": False, "discountRate": -0.25/100, "Ogden": 8, "claimants": [claimant, dependent1,nondependent3]}}
...
spreadsheet_id='1MADead62q5c8M0-O1Xuf-30aYy93FgfrrMM4vFKkmzw'
service_file_path="/Users/William/Dropbox (Personal)/Python/Code/pyCharm/Ogden8/pro-tracker-360811-236cce02c285.json"
sheet_name='Sheet1'
...
def joint_lives_table(start_age,end_age,sexes):
    ...
    for h in range(start_age,end_age): #column
        print (h)
        husband = {'name': "husband", 'age': h, 'sex': first_sex, 'dependenton': 'wife','dataSet': Ogden8,'retirement': 78} #create husband of age h
        colJLE=[]
        colJLM=[]
        for w in range (start_age,end_age): #row
            print (w)
            wife = {'name': "wife", 'age': w, 'sex': second_sex, 'dataSet': Ogden8, 'dependenton': 'husband', 'retirement': 78} #create wife of age h
            eg={"rows": [], 'game': {"trialDate":datetime(2022,8,25),"projection":True,"autoYrAttained": False, "discountRate": -0.25/100, "Ogden": 8, "claimants": [wife,husband]}}
            g=game(eg)
...
print(g.getClaimant('Norman').M(76,90, freq='Y'))
...
joint_lives_table(0,100,'MM')
joint_lives_table(0,100,'MF')
joint_lives_table(0,100,'FF')
